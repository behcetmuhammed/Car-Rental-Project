...
public class CarManager {
    ...
    public boolean update(Car car) {
        if (this.getById(car.getId()) == null) {
            Halper.showMessage(car.getId() + " ID kayıtlı araç bulunamadı");
            return false;
        }
    ...
    public boolean delete(int id) {
        if (this.getById(id) == null) {
            Halper.showMessage(id + " ID Kayıtlı araç Bulunamadı!");
            return false;
        }
        return this.carDao.delete(id);
    }

    //Araçlar içinde arama yapan metod
    public ArrayList<Car> searchForBooking(String strt_date, String fnsh_date, Model.Type type, Model.Gear gear, Model.Fuel fuel) {
        ...
        ArrayList<Car> searchCarList = this.carDao.selectByQuery(query);
        ...
        String bookOrWhereStr = String.join(" OR ", bookOrWhere); //bookOrWhere listesinin içindeki elemanları aralarına "Or" koyarak birleştirir.
        ...
        searchCarList.removeIf(car -> busyCarId.contains(car.getId()));
        //busyCarId: dolu olan (meşgul) araçlar
        //searchCarList: boş araçlar
